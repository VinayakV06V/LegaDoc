...
Resolves OCR spatial disconnection by:
1. Filtering overlapping detections via spatial IoU Non-Maximum Suppression (NMS).
2. Clustering bounding boxes into discrete horizontal rows by adaptive vertical tolerance.
3. Sorting within each row left-to-right to preserve column structure and natural reading order.
4. Extracting the 12 canonical FIR header fields across bilingual (English & Hindi) templates.
"""
...
def deduplicate_boxes_nms(
    ...
    Keeps the highest-confidence box or larger encompassing detection.
    """
    if not boxes:
        return []

    # Sort primarily by confidence descending, secondarily by area descending
    def sort_key(b: Dict[str, Any]) -> Tuple[float, int]:
...
def extract_bilingual_fir_fields(
    ...
    elif "HARYANA" in upper_full or "अम्बाला" in raw_text or "हिसार" in raw_text:
    ...
    # --- 1. FIR Number ---
    m_fir = re.search(
        r"(?i)(?:FIR\s*N[oO0]\.?|एफ\.?आई\.?आर\.?\s*सं\.?|प्रथम\s*सूचना\s*रिपोर्ट(?:\s*सं\.?)?)[:\s]*([0-9]{1,12}(?:/[0-9]{2,4})?)",
        raw_text,
    )
    if m_fir:
    ...
    m_dist = re.search(
        r"(?i)(?:District|जिला)[:\s]*([^|\n]+?)(?=\s*\||\s*P\.?S\.?[:\s]|\s*Police\s+Station|\s*थाना|\s*Year|\s*वर्ष|$)",
        raw_text,
    )
    if m_dist:
    ...
    m_ps = re.search(
        r"(?i)(?:P\.?S\.?|Police\s+Station|थाना)[:\s]*([^|\n]+?)(?=\s*\||\s*Year|\s*वर्ष|\s*FIR|$)",
        raw_text,
    )
    if m_ps:
    ...
    m_year = re.search(r"(?i)(?:Year|वर्ष)[:\s]*([12][09][0-9]{2})", raw_text)
    if m_year:
    ...
    m_date = re.search(r"(?i)(?:Date|दिनांक)[:\s]*([0-3]?[0-9][\/\-\.][01]?[0-9][\/\-\.][12][09][0-9]{2})", raw_text)
    if m_date:
        parsed["registration_date"] = m_date.group(1).strip()

    m_time = re.search(
        r"(?i)(?:Time\s*(?:From)?|समय)[:\s]*([0-2]?[0-9]:[0-5][0-9](?:\s*(?:hrs|nrs|am|pm))?)",
        raw_text,
    )
    if m_time:
    ...
    for m in re.finditer(r"(?i)(?:BNS|Bharatiya\s+Nyaya\s+Sanhita).*?(?:2023\s*)?([1-9][0-9]{1,2}(?:\([0-9a-zA-Z]+\))?)", raw_text):
        sec = m.group(1).strip()
        if sec and f"BNS {sec}" not in sections:
            sections.append(f"BNS {sec}")
    # Search for IPC sections
    for m in re.finditer(r"(?i)(?:IPC|Indian\s+Penal\s+Code|भा\.दं\.सं).*?([1-9][0-9]{1,2}(?:\([0-9a-zA-Z]+\))?)", raw_text):
        sec = m.group(1).strip()
        if sec and f"IPC {sec}" not in sections:
            sections.append(f"IPC {sec}")
    # Section/s heading directly followed by digits
    if not sections:
        for m in re.finditer(r"(?i)(?:Section/s|Sections|धाराएं|धारा)[:\s]*([1-9][0-9]{1,2}(?:\([0-9a-zA-Z]+\))?)", raw_text):
            sec = m.group(1).strip()
    ...
    m_comp = re.search(
        r"(?i)(?:Complainant\s*/\s*Informant|शिकायतकर्ता|वादी)[\s\S]*?(?:Name|नाम)?[:\s]*([A-Z][a-zA-Z\s\.]+(?:s\/o|w\/o|d\/o|LT\.\s*Sh\.)?[^|\n,\(\)]+)",
        raw_text,
    )
    if m_comp:
        comp_clean = m_comp.group(1).strip(" !:,-|")
        comp_clean = re.sub(r"(?i)^Name\s*[:\s]*", "", comp_clean).strip()
        if len(comp_clean) > 3:
            parsed["complainant"] = comp_clean

    # --- 10. Address ---
    m_addr = re.search(r"(?i)(?:Address|पता)[:\s]*([^|\n]+(?:DELHI|HARYANA|ROAD|NAGAR|COLONY)?[^|\n]*)", raw_text)
    if m_addr:
        parsed["address"] = m_addr.group(1).strip(" !:,-|")

    # --- 11. Place of Occurrence ---
    m_poc = re.search(
        r"(?i)(?:Place\s+of\s+Occurrence|घटना\s+स्थल)[\s\S]*?(?:Address[:\s]*)?([^|\n]+(?:PARK|ROAD|STATION|DELHI|HARYANA|PANDAL)[^|\n]*)",
        raw_text,
    )
    if m_poc:
        poc_clean = m_poc.group(1).strip(" !:,-|")
        poc_clean = re.sub(r"(?i)^(?:Place\s+of\s+Occurrence|Direction\s+and\s+Distance)[\s\S]*?Address[:\s]*", "", poc_clean).strip()
        if len(poc_clean) > 5:
    ...
    if m_desc:
        parsed["incident_description"] = m_desc.group(1).strip(" !:,-|")
